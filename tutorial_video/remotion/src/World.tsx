...
import { AbsoluteFill, useCurrentFrame, interpolate, Easing, OffthreadVideo, staticFile, Sequence, Audio } from "remotion";
import { VO } from "./voiceover";
...
const RAW: Omit<Seg, "start" | "end" | "clipStart">[] = [
  ...
  { id: "notebook", dur: 560, x: 13200, scale: 1.06 },
  { id: "edaDemo", dur: 1115 + LEAD + TAIL, x: 15600, scale: 1.0, clip: "clips/vidEDA.mp4", clipFrames: 1115 },
  { id: "modelDemo", dur: 1044 + LEAD + TAIL, x: 18000, scale: 1.0, clip: "clips/vidModel.mp4", clipFrames: 1044 },
  { id: "verifyDemo", dur: 871 + LEAD + TAIL, x: 20400, scale: 1.0, clip: "clips/vidA.mp4", clipFrames: 871 },
  { id: "rules", dur: 430, x: 22800, scale: 1.08 },
  { id: "start", dur: 520, x: 25200, scale: 1.04 },
  { id: "close", dur: 520, x: 27600, scale: 1.0 },
];
const TRAVEL = 28;
const SEG: Seg[] = [];
{
...
// ---- demo captions — voice-driven, generated by voiceover/generate.py -----
...
const Notebook: React.FC<{ local: number }> = ({ local }) => {
  const cells = [
    ["1", "載入資料 · 定義 outcome"],
    ["2", "整理變數 · 處理缺失"],
    ["3", "建立模型 · 印出 OR 表"],
  ...
  return (
    <Frame x={byId("notebook").x}>
      <div style={{ position: "absolute", left: 360, top: 210, ...rise(local, 0, 16) }}>
        <Eyebrow>地基四 · 寫成 notebook</Eyebrow>
        <div style={{ fontSize: 48, fontWeight: 900, marginTop: 12 }}>每一步，寫成 step-by-step notebook</div>
      </div>
      <div style={{ position: "absolute", left: 360, top: 380, width: 760, borderRadius: 18, background: "rgba(12,18,38,.9)", border: "1px solid rgba(120,150,220,.3)", overflow: "hidden", ...rise(local, 24, 18) }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 20px", background: "rgba(120,150,220,.12)", fontFamily: MONO, fontSize: 24, color: "#cfe0ff" }}>
          <span style={{ width: 13, height: 13, borderRadius: "50%", background: RED }} />
          <span style={{ width: 13, height: 13, borderRadius: "50%", background: GOLD }} />
          <span style={{ width: 13, height: 13, borderRadius: "50%", background: GREEN }} />
          <span style={{ marginLeft: 12 }}>03_modeling.ipynb</span>
        </div>
        {cells.map((c, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 18, padding: "18px 22px", borderTop: "1px solid rgba(120,150,220,.12)", opacity: grow(local, 36 + i * 12, 14) }}>
            <span style={{ fontFamily: MONO, fontSize: 22, color: "#7f8bad" }}>In [{c[0]}]:</span>
            <span style={{ fontSize: 28, color: "#dbe3f5" }}>{c[1]}</span>
          </div>
        ))}
      </div>
      <div style={{ position: "absolute", left: 1200, top: 430, width: 380, ...rise(local, 120, 20) }}>
        <div style={{ fontSize: 30, lineHeight: 1.7, color: "#dbe3f5" }}>
          要 AI <span style={{ color: ACCENT, fontWeight: 800 }}>寫成 notebook</span>，不是在對話裡丟答案。
        </div>
        <div style={{ fontSize: 26, lineHeight: 1.7, color: "#8d97b0", marginTop: 22 }}>
          <span style={{ color: GREEN }}>可重跑</span> · <span style={{ color: GREEN }}>留證據</span> · <span style={{ color: GREEN }}>能逐格檢查</span>
        </div>
      </div>
    </Frame>
  );
...
const Rules: React.FC<{ local: number }> = ({ local }) => {
  const cards = [
    ["①", "一定要自己逐格檢查", "逐格跑過、看懂每一步，確認程式真的照你要的跑、沒有黑箱。", ACCENT],
    ["②", "最後由你拍板", "AI 不會替你做臨床判斷，負責的永遠是主治醫師。", GREEN],
  ];
  return (
...
export const World: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ background: "radial-gradient(1500px 1000px at 50% 42%, #101a3a, #070b16 72%)", fontFamily: FONT, color: "#eef2fb", overflow: "hidden" }}>
      {/* voiceover — one ElevenLabs clip per script entry, sequenced by frame */}
      {VO.map((v, i) => (
      ...
      <div style={{ position: "absolute", left: 0, top: 0, width: WORLD_W, height: 1080, transform: `translate(${tx}px,${ty}px) scale(${s})`, transformOrigin: "0 0" }}>
        ...
        <Notebook local={f - byId("notebook").start} />
        <Rules local={f - byId("rules").start} />
        <Start local={f - byId("start").start} />

        {/* demo monitors */}
        <Monitor seg={byId("edaDemo")} cues={CAPTIONS.eda} tag="● EDA" />
        <Monitor seg={byId("modelDemo")} cues={CAPTIONS.model} tag="● MODELING" />
        <Monitor seg={byId("verifyDemo")} cues={CAPTIONS.verify} tag="● VERIFY" />

        {/* station labels visible on the wide reveal */}
        {[["原始資料", byId("open").x], ["指揮", byId("flow").x], ["結構", byId("struct").x], ["Git", byId("git").x], ["個資", byId("priv").x], ["Notebook", byId("notebook").x], ["EDA", byId("edaDemo").x], ["建模", byId("modelDemo").x], ["驗收", byId("verifyDemo").x], ["鐵則", byId("rules").x], ["起步", byId("start").x]].map((l, i) => (
      ...
      <div style={{ position: "absolute", left: 0, top: 150, width: "100%", textAlign: "center", opacity: interpolate(f, [close.start, close.start + 40, close.start + 230, close.start + 280], [0, 1, 1, 0], clamp) }}>
        <div style={{ fontSize: 52, fontWeight: 900 }}>臨床研究分析，這樣做，本質上就是一場 <span style={{ color: GREEN }}>vibe coding</span>。</div>
      </div>
